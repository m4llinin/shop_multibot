...
from config.config import session, main_bot, client
...
@client.pay_handler()
async def cryptopay_invoice_paid(update: Update, app) -> None:
    order_id = int(update.payload.payload)
    try:
        texts = await load_texts()
        order = await Database.ShopBot.get_order(order_id)
        good = await Database.ShopBot.get_good_by_id(order.good_id)
        shop = await Database.ShopBot.get_shop_by_name(order.shop_id)
        bot = Bot(token=shop.token, session=session)
